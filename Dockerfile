...
#   yourimage
CMD ["sh", [---gg----] "-c", "turnserver --lt-cred-mech --fingerprint --user=${TURN_USERNAME}:${TURN_PASSWORD} --realm=${REALM} --listening-port=3478 --min-port=49152 --max-port=65535 --no-multicast-peers --no-loopback-peers --log-file=stdout --external-ip=${EXTERNAL_IP:-$(curl -s https://api.ipify.org)}"]
